...
function rebuild( d , root ){
	...
	for(x=0,len = d.nodes.length; x<len; x++){
...
function Store( options ){
	...
	this.client           = new Etcd(options.hosts||['127.0.0.1:4001'] );
}
...
Store.prototype.load = function load( cb ){
	this.client.get('/' + this.namespace, {recursive:true}, function(err, data ){
		(cb||NOOP)(err, err ? null : this.store = rebuild( data.node )[this.namespace] );
	}.bind( this ));
};
...
Store.prototype.loadSync = function load( cb ){
	...
	return this.store = rebuild( response.body.node )[this.namespace]
};
...
Store.prototype.saveSync = function save( cb ){
	...
	for(var idx=0, len=keys.length; idx<len; idx++ ){
		...
		switch(code){
			case 102:
				this.client.rmdirSync( full.key, {recursive:true} );
